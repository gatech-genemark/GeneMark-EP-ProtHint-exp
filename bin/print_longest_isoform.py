...
def printLongest(input, longestTranscripts):
    for row in csv.reader(open(input), delimiter='\t'):
        if (row[2] == 'CDS' or row[2] == 'start_codon' or row[2] == 'stop_codon' or row[2] == 'intron'):
            gene = extractFeature(row[8], 'gene_id')
            transcript = extractFeature(row[8], 'transcript_id')
...
def main():
    args = parseCmd()
    transcriptLengths = computLengths(args.input)
...
def parseCmd():
    ...
    parser = argparse.ArgumentParser(description='Print longest isoforms in a gtf file')
